...
class LinearClassifierAdaptationNetwork(nn.Module):
    ...
    def forward(self, representation):
        ...
        class_weight_means = self.weight_means_processor(representation)
        class_bias_means = self.bias_means_processor(representation)

        # Save the parameters as torch tensors (matrix and vector) and add to dictionary
        classifier_param_dict['weight_mean'] = class_weight_means
        classifier_param_dict['bias_mean'] = class_bias_means
...
class FilmArAdaptationNetwork(nn.Module):
    ...
    def forward(self, x, task_representation):
        ...
        # x = torch.cat([x, task_representation.repeat(x.size(0), 1)], dim=1)
        ...
        for layer, hn_layer in enumerate(self.layers):
            param_dicts.append(hn_layer(z, task_representation))
...
class FilmArLayerNetwork(nn.Module):
    ...
    def forward(self, x, task_representation):
        ...
        """
        x = self.shared_layer(x)
        x = torch.mean(x, dim=1, keepdim=True)  # pooling over instances
        x = self.shared_layer_post(x)
        task_representation = task_representation[:, None, :].repeat(1, x.size(1), 1)
        x = torch.cat([x, task_representation], dim=-1)
        block_params = []
        for block in range(self.num_blocks):
            block_param_dict = {
                'gamma1': self.gamma1_processors[block](x).squeeze(1) * self.gamma1_regularizers[block] +
                          torch.ones_like(self.gamma1_regularizers[block]),
                'beta1': self.beta1_processors[block](x).squeeze(1) * self.beta1_regularizers[block],
                # 'gamma2': self.gamma2_processors[block](x).squeeze() * self.gamma2_regularizers[block] +
                #           torch.ones_like(self.gamma2_regularizers[block]),
                # 'beta2': self.beta2_processors[block](x).squeeze() * self.beta2_regularizers[block]
            }
